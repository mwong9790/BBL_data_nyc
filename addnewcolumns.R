...
pluto_all_short <- subset(pluto_all,select=c(BBL,CT2010.y,CBlockGroup.x,YearBuilt.x,UnitsRes.x,SUnits_by_bbl.x,HViol_by_bbl.x,Total,White,Black,Native_American,Asian,Hawaiian_or_Pacific_Islander,Other_Race,Two._Races,Two._Races_not_Other,Three._Races_not_Other,Median_HH_Income,nycha))

#renaming
pluto_all_short <- pluto_all_short %>% rename(CT2010 = CT2010.y, CBlockGroup = CBlockGroup.x, YearBuilt=YearBuilt.x,UnitsRes=UnitsRes.x,SUnits=SUnits_by_bbl.x,HViol=HViol_by_bbl.x,TotalRace=Total,TwoRaces=Two._Races,TwoRacesNotOther=Two._Races_not_Other,ThreeRacesNotOther=Three._Races_not_Other)
